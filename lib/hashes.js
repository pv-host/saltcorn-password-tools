 ...
 *   - CRAM-MD5      Dovecot format: 64 hex chars = HMAC-MD5 key context
 *
...
const { hashCramMd5, verifyCramMd5 } = require("./cram-md5");

const SCHEMES = ["BLF-CRYPT", "SHA512-CRYPT", "PBKDF2", "CRAM-MD5"];
// Aliases accepted on verify (e.g. Dovecot also documents {HMAC-MD5}).
const SCHEME_ALIASES = { "HMAC-MD5": "CRAM-MD5" };
 ...
 * @param {string} plain
 * @param {object} opts
 * @param {"BLF-CRYPT"|"SHA512-CRYPT"|"PBKDF2"|"CRAM-MD5"} opts.scheme
...
async function hashPassword(plain, opts) {
  ...
  switch (o.scheme) {
    ...
    case "CRAM-MD5": {
      // Dovecot CRAM-MD5 stores the HMAC-MD5 key setup state as 32 raw
      // bytes (opad state || ipad state) rendered as 64 lowercase hex chars.
      const hash = hashCramMd5(plain);
      return withScheme("CRAM-MD5", hash, o.withPrefix);
    }
    default:
...
async function verifyPassword(plain, stored) {
  ...
  const resolved = scheme ? (SCHEME_ALIASES[scheme] || scheme) : null;
  const detected = resolved || detectSchemeFromHash(hash);
  ...
  try {
    switch (detected) {
      ...
      case "CRAM-MD5": {
        // Body must be 64 hex chars; verifyCramMd5 handles timing-safe compare.
        return verifyCramMd5(plain, hash);
      }
      default:
        return false;
    }
...
function detectSchemeFromHash(hash) {
  ...
  // Note: bare 64-hex-char strings are NOT auto-detected as CRAM-MD5 -
  // callers must include the {CRAM-MD5} (or {HMAC-MD5}) prefix, since
  // 64 hex chars alone are ambiguous (SHA-256, other MD-based schemes, ...).
  return null;
}
